...
챗봇이 "정보 조회 + 답변 생성" 까지라면, 에이전트는 "정보 조회 + 의사결정 +
실제 액션" 까지 갑니다. 가장 명확한 사례는 **도서관 좌석 자동 예약**.

```text
사용자: "도서관 4층 창가 자리로 4시간 예약해줘"
  ↓
에이전트:
  1. get_library_seat_status() → 실시간 좌석 조회
  2. 사용자 선호 (4층, 창가, 4시간) 와 매칭되는 후보 추출
  3. 사용자에게 confirmation: "412번 자리로 16:00–20:00 예약할게요?"
  4. 사용자 yes → reserve_library_seat(id=412, duration=4h) 액션 호출
  5. 결과 보고: 성공 / 부분 실패 / 취소
```

write 성격의 액션 도구 ( `reserve_library_seat`, `submit_assignment` 등)
는 **모두** 다음 규약을 따릅니다.

- 사용자 명시 confirmation 필수
- 실행 전 dry-run 결과 표시
- 모든 요청·결과 audit log 에 기록
- 실패·부분 성공·취소를 사용자에게 명확히 반환
- 비밀번호·세션·토큰 등 비밀 정보 log 에 절대 안 남김

자세한 액션 도구 정책은 [`docs/security.md`](security.md) 와
[`docs/mcp-tools.md`](mcp-tools.md) §8 참고.
...
| **Phase 4** | AI 에이전트 — 도서관 좌석 자동 예약, write 액션 도구 정책 구체화 | 📋 계획 중 |
...
- `reserve_library_seat` 같은 write 액션 도구 추가
- 액션 도구 confirmation / dry-run / audit log 시스템
- 챗봇 위에 "도구 결과 → 액션 도구 호출 결정" agent loop 구현
- 안전 가드 — 같은 액션 중복 실행 방지, 실패 시 graceful rollback
...
| Phase 2 | Claude Desktop 에서 ssuAI MCP 붙이고 "도서관에 자리 있어?" 답변 |
| Phase 3 | 인증된 사용자가 챗봇에서 "내 다음 수업 뭐야?" 답변 받음 |
| Phase 4 | 챗봇에서 "도서관 자리 예약해줘" → 사용자 confirmation → 실제 예약 완료 |
